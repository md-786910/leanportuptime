...
export default function SiteOverviewTab({ site, summary, checks = [], period = '24h', readOnly = false, sslData: externalSsl, securityData: externalAudit }) {
  ...
  const kpis = [
    ...
    { label: 'Avg Response', value: formatResponseTime(summary?.avgResponseTime), color: 'text-brand-600 dark:text-brand-400' },
    { label: 'Total Checks', value: summary?.totalChecks || 0, color: 'text-gray-900 dark:text-white' },
    ...
    { label: 'Max Response', value: formatResponseTime(summary?.maxResponseTime), color: 'text-gray-900 dark:text-white' },
    { label: 'Avg TTFB', value: formatResponseTime(summary?.avgTtfb), color: 'text-gray-900 dark:text-white' },
  ];
  ...
  let secColor = 'text-gray-500';
  let secBarColor = 'bg-gray-400';
  if (secScore != null) {
  ...
  return (
    <div className="space-y-6">
          ...
          {sslData?.validTo ? (
            <div className={`rounded-lg p-3 ${sslBg}`}>
              <div className="flex items-center justify-between mb-2">
                <span className={`text-lg font-bold ${sslColor}`}>
              ...
              <div className="space-y-1 text-xs text-gray-600 dark:text-gray-400">
                <div className="flex justify-between">
                  <span>Issuer</span>
                  <span className="text-gray-900 dark:text-gray-200">{sslData.issuer || '—'}</span>
                </div>
                <div className="flex justify-between">
                  <span>Expires</span>
                  <span className="text-gray-900 dark:text-gray-200">{formatDate(sslData.validTo)}</span>
                </div>
                <div className="flex justify-between">
                  <span>Protocol</span>
                  <span className="text-gray-900 dark:text-gray-200">{sslData.protocol || '—'}</span>
                </div>
              </div>
            </div>
          ) : (
            <p className="text-sm text-gray-500 dark:text-gray-400 py-4 text-center">No SSL data yet</p>
          )}
        </Card>
          ...
          {secScore != null ? (
            <div>
              <div className="flex items-end gap-2 mb-2">
                <span className={`text-3xl font-bold ${secColor}`}>{secScore}</span>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 py-4 text-center">No security scan yet</p>
          )}
        </Card>
      </div>
      ...
      {recentChecks.length > 0 && (
        <Card padding="none">
          <div className="px-4 py-3 border-b border-gray-100 dark:border-gray-700">
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Recent Checks</h3>
          </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100 dark:border-gray-700">
                  ...
                  <th className="text-left py-2 px-4 font-medium text-gray-500 text-xs">HTTP</th>
                  <th className="text-left py-2 px-4 font-medium text-gray-500 text-xs">Response</th>
                  <th className="text-left py-2 px-4 font-medium text-gray-500 text-xs">TTFB</th>
                  <th className="text-left py-2 px-4 font-medium text-gray-500 text-xs">Location</th>
                </tr>
              </thead>
              <tbody>
                {recentChecks.map((c) => (
                  <tr key={c._id} className="border-b border-gray-50 dark:border-gray-800">
                    <td className="py-2 px-4 text-gray-700 dark:text-gray-300 text-xs">{formatRelative(c.timestamp)}</td>
                    <td className="py-2 px-4">
                      <span className={`text-xs font-medium ${
                        c.status === 'up' ? 'text-emerald-600' : c.status === 'down' ? 'text-red-600' : 'text-amber-600'
                      }`}>{c.status}</span>
                    </td>
                    <td className="py-2 px-4 text-gray-700 dark:text-gray-300">{c.httpStatus || '—'}</td>
                    <td className="py-2 px-4 text-gray-700 dark:text-gray-300">{formatResponseTime(c.responseTime)}</td>
                    <td className="py-2 px-4 text-gray-700 dark:text-gray-300">{formatResponseTime(c.ttfb)}</td>
                    <td className="py-2 px-4">
      ...
      {/* Site Info */}
      <Card>
        <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100 mb-3">Site Info</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
          <div className="flex justify-between">
            <span className="text-gray-500 dark:text-gray-400">URL</span>
            <a href={site.url} target="_blank" rel="noopener noreferrer" className="text-brand-600 dark:text-brand-400 hover:underline truncate ml-4">{site.url}</a>
          </div>
          <div className="flex justify-between">
            <span className="text-gray-500 dark:text-gray-400">Check Interval</span>
            <span className="text-gray-900 dark:text-gray-100">{intervalLabel}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-gray-500 dark:text-gray-400">Last Checked</span>
            <span className="text-gray-900 dark:text-gray-100">{formatRelative(site.lastCheckedAt)}</span>
          </div>
          <div className="flex justify-between">
          ...
          <div className="flex justify-between">
            ...
            <span className={site.consecutiveFailures > 0 ? 'text-red-600 dark:text-red-400 font-medium' : 'text-gray-900 dark:text-gray-100'}>{site.consecutiveFailures || 0}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-gray-500 dark:text-gray-400">Created</span>
            <span className="text-gray-900 dark:text-gray-100">{formatDate(site.createdAt)}</span>
          </div>
