...
export default function CompareChannelsModal({ isOpen, onClose, siteId, currentChannels = [], currentLabel = 'Current Period' }) {
  ...
  const customCurrentChannels = customCurrentData?.details?.channels || [];
  const effectiveCurrentChannels = cmp.isCustom ? customCurrentChannels : currentChannels;
  ...
  const merged = useMemo(() => {
    ...
    for (const c of effectiveCurrentChannels) {
    ...
    return arr;
  }, [effectiveCurrentChannels, compareChannels]);

  const totalCurrent = useMemo(
    () => effectiveCurrentChannels.reduce((s, c) => s + (c.sessions || 0), 0),
    [effectiveCurrentChannels],
  );
