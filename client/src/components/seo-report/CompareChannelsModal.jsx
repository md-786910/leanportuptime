...
export default function CompareChannelsModal({ isOpen, onClose, siteId, currentChannels = [], currentLabel = 'Current Period' }) {
  ...
  const compareChannels = compareData?.details?.channels || [];
  ...
  // Merge channel rows by channel name.
  const merged = useMemo(() => {
    const byName = new Map();
    for (const c of currentChannels) {
      byName.set(c.channel, { name: c.channel, current: c.sessions || 0, previous: 0 });
    }
    for (const c of compareChannels) {
      const row = byName.get(c.channel) || { name: c.channel, current: 0, previous: 0 };
      row.previous = c.sessions || 0;
      byName.set(c.channel, row);
    }
    const arr = [...byName.values()];
    arr.sort((a, b) => Math.max(b.current, b.previous) - Math.max(a.current, a.previous));
    return arr;
  }, [currentChannels, compareChannels]);

  const totalCurrent = useMemo(
    () => currentChannels.reduce((s, c) => s + (c.sessions || 0), 0),
    [currentChannels],
  );
  const totalCompare = useMemo(
    () => compareChannels.reduce((s, c) => s + (c.sessions || 0), 0),
    [compareChannels],
  );
  ...
  return (
    <Drawer
      isOpen={isOpen}
      onClose={onClose}
      title="Compare Traffic by Channel"
      width="lg"
      footer={
      ...
      <div className="space-y-5">
        ...
        <div className="grid grid-cols-3 gap-3">
          <div className="rounded-xl border border-brand-outline-variant dark:border-brand-outline px-3 py-2.5">
            <div className="text-[10px] uppercase tracking-wider text-brand-outline font-label">{currentLabel}</div>
            <div className="text-lg font-bold tabular-nums text-brand-on-surface dark:text-white font-headline">{fmtNumber(totalCurrent)}</div>
          </div>
          <div className="rounded-xl border border-brand-outline-variant dark:border-brand-outline px-3 py-2.5">
            <div className="text-[10px] uppercase tracking-wider text-brand-outline font-label truncate">{headerLabel || '—'}</div>
            <div className="text-lg font-bold tabular-nums text-brand-on-surface-variant dark:text-brand-outline font-headline">{available ? fmtNumber(totalCompare) : (isLoading ? '…' : '—')}</div>
          </div>
          <div className="rounded-xl border border-brand-outline-variant dark:border-brand-outline px-3 py-2.5">
            <div className="text-[10px] uppercase tracking-wider text-brand-outline font-label">Δ Sessions</div>
        ...
        {merged.length === 0 ? (
          <div className="text-center py-8 rounded-xl border border-dashed border-brand-outline-variant dark:border-brand-outline">
            <p className="text-sm text-brand-on-surface-variant dark:text-brand-outline">No channels to compare.</p>
          </div>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-brand-surface-container-low dark:bg-brand-on-surface/50">
                  <th className="text-left  py-2.5 px-3 font-medium text-brand-on-surface-variant dark:text-brand-outline text-[10px] uppercase tracking-wider font-label">Channel</th>
                  ...
                  return (
                    <tr key={row.name} className={`${i > 0 ? 'border-t border-gray-50 dark:border-brand-outline' : ''} hover:bg-brand-surface-container-low/40 dark:hover:bg-brand-on-surface/20 transition-colors`}>
                      ...
                      <td className="py-2 px-3 text-center text-sm font-bold tabular-nums text-brand-on-surface-variant dark:text-brand-outline font-headline whitespace-nowrap">
                        {available ? fmtNumber(row.previous) : (isLoading ? '…' : '—')}
                      </td>
