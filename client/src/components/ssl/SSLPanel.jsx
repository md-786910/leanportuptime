...
export default function SSLPanel({ siteId, readOnly = false, sslData: externalSsl, historyData: externalHistory }) {
  const { ssl: fetchedSsl, isLoading } = useSSL(readOnly ? null : siteId);
  const { history: fetchedHistory, isLoading: histLoading } = useSSLHistory(readOnly ? null : siteId);
  const sslCheck = useSSLCheck(readOnly ? null : siteId);
